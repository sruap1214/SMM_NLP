...
with open('./models/newW2VWords.pkl', 'rb') as handle:
    palabrasW2V = pickle.load(handle)

with open('./models/newW2VVectors.pkl', 'rb') as handle:
    newW2VEmbeding = pickle.load(handle)
...
def embeddingW2V(palabra):
    wordW2V=searchWord(palabra)
    if wordW2V != 'notFound':
        vector=newW2VEmbeding[palabrasW2V.index(wordW2V),:]
    else:
        vector=np.zeros([300,])
    return vector
